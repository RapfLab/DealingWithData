...
os.chdir(Path0)
ListFiles = pd.read_csv('./RunMixHisto.csv')
...
for idx, row in ListFiles.iterrows():
    ...
    for entry in range(NumMeasure):
        ...
        DiamCol = dataname.iloc[:,entry*2].values.tolist()
        RadiusCol = [i/2 for i in DiamCol]
        
        MeasCol = dataname.iloc[:,(entry*2+1)].values.tolist()
        #print len(RadiusCol)
            ...
            if PeakDetected == True:
                ...
                if meas == 0:
                    ...
                    for peak in range(NumPeaks):
                        print "MEAS"+ str(meas) + " PEAK" + str(peak)
...
#                                PeakStatistics.iloc[meas,(peak+1)*3+2] = PercentPoly
    ...
    #print CleanedData
    CleanedFinalData.to_csv('./ExtractedData/MarkHisto/'+prename+'CleanOutput.csv')
    PeakStatistics.to_csv('./ExtractedData/MarkHisto/'+prename+'PeakStats.csv')
...
CompileData.to_csv('./ExtractedData/MarkHisto/'+'MixCompiledOutput.csv')
